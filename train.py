...
def train(retriever, llm_pipeline, device, num_epoches=3,num_inner_epoch = 1, 
          training_size=300, group_size=8,epsilon=0.2,beta=0.05):
    ...
    for epoch in range(num_epoches):
        ...
        for example_idx in tqdm(range(training_size)):
            ...
            icl_copy, icl_update, icl_retain = construct_icl_examples(i, demos, order, corpus_idx)
            ...
            for _ in range(group_size):
                ...
                eps = sample_retrieved_examples(retriever, query, sample_examples, 32)
                group.append(eps)
                ...
                edit_answer = generate_response(prompts_edit, llm_pipeline)
                ...
                para_answer = generate_response(prompts_para, llm_pipeline)
                ...
                retain_answer = generate_response(prompts_retain, llm_pipeline)
            ...
            for inner_epoch in range(num_inner_epoch):
                ...
                for g_idx in range(group_size):
                    ...
                    for t, (action, old_log_prob) in enumerate(trajectory):
                        ...
                        new_d = torch.distributions.Categorical(probs.to(device))
                        ref_d = torch.distributions.Categorical(ref_probs.to(device))

                        new_log_prob = new_d.log_prob(torch.tensor(action_idx).to(device))
                        ratio = torch.exp(new_log_prob - old_log_prob.to(device))                        
...
if __name__ == '__main__':
    ...
    num_samples = 300
    set_seed(42)
    training_data = [random.randint(0,1999) for i in range(num_samples)]
    stochastic()
    ...
    train(retriever, llm_pipeline, device)
